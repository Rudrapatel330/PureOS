...
static rsdp_descriptor_t *find_rsdp(void) {
  ...
  // Search BIOS ROM from 0xE0000 to 0xFFFFF
...
static void parse_madt(madt_t *madt) {
  ...
  while (ptr < end) {
    ...
    if (record->type == 0) {
      ...
      if (lapic->flags & 1) { // Processors is enabled
        if (num_cpus < MAX_CPUS) {
          cpus[num_cpus].apic_id = lapic->apic_id;
          cpus[num_cpus].is_bsp =
              (num_cpus == 0)
                  ? 1
                  : 0; // First CPU is BSP? (Or check IA32_APIC_BASE MSR later)
          num_cpus++;
        }
      }
...
void acpi_shutdown(void) {
  ...
  // Method 1: Standard ACPI shutdown via PM1a control register
  if (acpi_pm1a_cnt) {
    // Enable ACPI mode if needed (write acpi_enable to SMI_CMD)
    if (acpi_smi_cmd && acpi_enable_val) {
      outb(acpi_smi_cmd, acpi_enable_val);
      // Wait a bit for ACPI to enable
      for (volatile int i = 0; i < 1000000; i++)
        ;
    }
    // SLP_TYPa = 5 (S5 soft-off), SLP_EN = bit 13
    // The exact SLP_TYP value should come from DSDT \_S5 object,
    // but 5 << 10 is the most common value for Bochs/QEMU
    outw(acpi_pm1a_cnt, (5 << 10) | ACPI_SLP_EN);
  }

  // Method 2: Bochs/QEMU specific shutdown ports
  outw(0xB004, 0x2000); // Bochs shutdown
  outw(0x604, 0x2000);  // QEMU shutdown

  // Method 3: Triple fault as last resort
