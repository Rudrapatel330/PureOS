...
typedef struct page_directory {
  uint64_t tables_physical[512]; // PT physical addresses
  page_table_t *tables[512];     // Virtual pointers
} page_directory_t;

typedef struct page_directory_pointer {
  uint64_t directories_physical[512]; // PD physical addresses
  page_directory_t *directories[512]; // Virtual pointers
} page_directory_pointer_t;

typedef struct pml4_table {
  uint64_t pdpt_physical[512];          // PDPT physical addresses
  page_directory_pointer_t *pdpts[512]; // Virtual pointers
  uint64_t phys_addr;                   // Physical address of this PML4
} pml4_table_t;
