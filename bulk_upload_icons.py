...
def upload_to_pure_data(file_path):
    ...
    with open(disk_path, 'r+b') as f:
        ...
        # 2. Find free clusters (or existing clusters if we're overwriting)
        # For simplicity, we'll always allocate new clusters and update the entry
        # A real implementation would free old clusters.
        ...
        if len(free_clusters) < needed_clusters:
            print(f"Error: Not enough free space for {file_name}.")
            return
        ...
        # 5. Create/Update Directory Entry
        f.seek(entry_offset)
        ...
        print(f"Uploaded {file_name} as {name_part.strip()}.{ext_part.strip()}")
...
def main():
    ...
    for filename in os.listdir(icons_dir):
        if filename.lower().endswith('.png'):
            upload_to_pure_data(os.path.join(icons_dir, filename))
            
    # Also upload the wallpaper
    if os.path.exists('src/fs/wallpaper_ultra.png'):
        upload_to_pure_data('src/fs/wallpaper_ultra.png')
    elif os.path.exists('src/fs/wallpaper.png'):
        upload_to_pure_data('src/fs/wallpaper.png')
